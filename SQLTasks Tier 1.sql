...
WHEN monthlymaintenance > 100 THEN 'expensive' END AS monthlymaintenance FROM Facilities;
...
####################Second Method##############################
SELECT DISTINCT CONCAT_WS( " ", firstname , surname ) AS member, f.name AS facility
FROM Members AS m
INNER JOIN Bookings AS b ON m.memid = b.memid
INNER JOIN Facilities AS f ON b.facid = f.facid
WHERE b.facid in (0,1)
...
FROM Members AS m
    ...
    INNER JOIN Facilities AS f ON b.facid = f.facid
WHERE 
    b.starttime >= '2012-09-04' AND b.starttime < '2012-09-05' AND 
